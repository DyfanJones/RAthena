...
test_data <- function(N = 10000L, seed = 142L) {
  ...
  data.table(
    id = 1:N,
    original = sapply(1:N, function(x) {
      paste(sample(letters, sample(5:10, 1)), collapse = ",")
    }),
...
output_test_data <- function(N = 10000L, temp_file) {
  ...
  fwrite(
    x = dt_list[, .(id, original, raw_string, json)],
    file = temp_file,
    quote = T
  )
}
...
output_test_error_data <- function(temp_file, N = 10000L, seed = 142L) {
  ...
  fwrite(dt, temp_file, quote = T)
}

data_type <- list(
  list(Name = "id", Type = "integer"),
  list(Name = "original", Type = "string"),
  list(Name = "raw_string", Type = "varbinary"),
  list(Name = "json", Type = "json")
)
...
test_that("Check if json has been correctly parser under chunk method", {
  ...
  RAthena::RAthena_options(json = "auto")
  RAthena::RAthena_options(binary = "character")

  dt1 <- RAthena:::athena_read.athena_data.table(
    method_1,
    test_file,
    data_type,
    con
  ...
  dt2 <- RAthena:::athena_read.athena_vroom(
    method_2,
    test_file,
    data_type,
    con
  ...
  expect_equal(
    jsonlite::toJSON(dt1$json[[1]], auto_unbox = T),
    jsonlite::toJSON(iris[1, 1:2])
  )
  expect_equal(
    jsonlite::toJSON(dt2$json[[1]], auto_unbox = T),
    jsonlite::toJSON(iris[1, 1:2])
  )
  unlink(test_file)
})
...
test_that("Binary and json conversion", {
  ...
  RAthena::RAthena_options(json = "auto")
  RAthena::RAthena_options(binary = "raw")
...
test_that("Check in conversion is turned off", {
  ...
  RAthena::RAthena_options(json = "character")
  RAthena::RAthena_options(binary = "character")
...
test_that("Custom json parser", {
  ...
  RAthena::RAthena_options(json = jsonlite::fromJSON)
  RAthena::RAthena_options(binary = "character")
...
test_that("Check if variable is returns as character when failed to convert", {
  ...
  expect_warning(
    dt <- RAthena:::athena_read.athena_data.table(
      method_1,
      test_file,
      data_type,
      con
...
test_that("Check accepted bigint options", {
  ...
  for (i in BigInt) {
    expect_invisible(RAthena::RAthena_options(bigint = i))
  }
  expect_error(RAthena::RAthena_options(bigint = "character2"))
})

test_that("Check accepted json options", {
  Json <- c("auto", "character")
  for (i in Json) {
    expect_invisible(RAthena::RAthena_options(json = i))
  }
...
test_that("Check if column is correctly parsed.", {
  ...
  expect_equal(
    result[[1]][1],
    "{\"var3\":[1,2,3],\"var4\":{\"var5\":[\"a\",\"b\",\"c\",\"d\",\"e\"]}}"
  )
