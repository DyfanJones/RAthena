...
#' @param file.type What file type to store data.frame on s3, RAthena currently supports `c("tsv", "csv", "parquet", "json")`. Default delimited file type is "tsv", in previous versions
...
#' @param compress \code{FALSE | TRUE} To determine if to compress file.type. If file type is `c("csv", "tsv")` then "gzip" compression is used, for file type "parquet"
...
Athena_write_table <-
  function(
    ...
    sapply(tolower(names(partition)), function(x) {
      if (x %in% tolower(names(value))) {
        stop(
          "partition ",
          x,
          " is a variable in data.frame ",
    ...
    if (max.batch < 0) {
      stop("`max.batch` has to be greater than 0", call. = F)
    }

    # use default s3_staging directory is s3.location isn't provided
    if (is.null(s3.location)) {
      s3.location <- conn@info$s3_staging
    }
    ...
    if (!is.null(partition) && is.null(names(partition))) {
      stop(
        "partition parameter requires to be a named vector or list",
        call. = FALSE
      )
    }
    ...
    if (found && !overwrite && !append) {
      stop(
        "Table ",
        db_name,
        " exists in database, and both overwrite and",
        " append are FALSE",
        call. = FALSE
      )
    }
    ...
    if (!found && append) {
      stop(
        "Table ",
        db_name,
        " does not exist in database and append is set to TRUE",
        call. = FALSE
      )
    }
    ...
    if (found && append) {
      ...
      File.Type <- switch(
        tbl_info$Parameters$serde.serialization.lib,
        "org.apache.hadoop.hive.serde2.lazy.LazySimpleSerDe" = switch(
          tbl_info$Parameters$serde.param.field.delim,
          "," = "csv",
          "\t" = "tsv",
        ...
        stop(
          "Unable to append onto table: ",
          db_name,
          "\n",
          tbl_info$Parameters$serde.serialization.lib,
          ": Is currently not supported by RAthena",
          call. = F
        )
      )
      ...
      compress = switch(
        File.Type,
        "parquet" = {
          if (is.null(tbl_info$Parameters$parquet.compress)) {
            FALSE
          } else {
            if (tolower(tbl_info$Parameters$parquet.compress) == "snappy") {
              TRUE
            } else {
              stop(
                "RAthena currently only supports SNAPPY compression for parquet",
                call. = F
              )
            }
          }
        ...
        "tsv" = {
          if (is.null(tbl_info$Parameters$compressionType)) {
            ...
            if (tolower(tbl_info$Parameters$compressionType) == "gzip") {
              TRUE
            } else {
              stop(
                "RAthena currently only supports gzip compression for tsv",
                call. = F
              )
            }
          }
        ...
        "csv" = {
          if (is.null(tbl_info$Parameters$compressionType)) {
            ...
            if (tolower(tbl_info$Parameters$compressionType) == "gzip") {
              ...
              stop(
                "RAthena currently only supports gzip compression for csv",
                call. = F
              )
            }
          }
        ...
        "json" = {
          if (is.null(tbl_info$Parameters$compressionType)) {
            ...
            if (!is.null(tbl_info$Parameters$compressionType)) {
              stop(
                "RAthena currently doesn't support compression for json",
                call. = F
              )
            }
          }
      ...
      if (file.type != File.Type) {
        warning(
          'Appended `file.type` is not compatible with the existing Athena DDL file type and has been converted to "',
          File.Type,
          '".',
          call. = FALSE
        )
      ...
      s3.location <- tolower(tbl_info$Parameters$location)
      file.type <- File.Type
    }

    # return original Athena Types
    if (is.null(field.types)) {
      field.types <- dbDataType(conn, value)
    }
...
setMethod(
  "dbWriteTable",
  c("AthenaConnection", "character", "data.frame"),
  function(
    conn,
    name,
    value,
...
setMethod(
  "dbWriteTable",
  c("AthenaConnection", "Id", "data.frame"),
  function(
    conn,
    name,
    value,
...
setMethod(
  "dbWriteTable",
  c("AthenaConnection", "SQL", "data.frame"),
  function(
    conn,
    name,
    value,
...
#' @param file.type What file type to store data.frame on s3, RAthena currently supports `c("csv", "tsv", "parquet", "json")`.
...
setMethod(
  ...
  function(
    ...
    if (file.type != "parquet") {
      ...
      posixct_cols <- names(Value)[sapply(col_types, function(x) {
        "POSIXct" %in% x
      })]
    ...
    for (col in list_cols) {
      set(Value, j = col, value = col_to_ndjson(Value, col))
    }
    ...
    switch(
      file.type,
      csv = {
        # changed special character from "," to "." to avoid issue with parsing delimited files
        for (col in special_char) {
          set(Value, j = col, value = gsub(",", "\\.", Value[[col]]))
        }
        info_msg(
          "Special character \",\" has been converted to \".\" to help with Athena reading file format csv"
        )
      },
      tsv = {
        # changed special character from "\t" to " " to avoid issue with parsing delimited files
        ...
        info_msg(
          "Special characters \"\\t\" has been converted to \" \" to help with Athena reading file format tsv"
        )
      }
    )
...
setMethod(
  ...
  function(
    ...
    # use default s3_staging directory is s3.location isn't provided
    if (is.null(s3.location)) {
      s3.location <- con@info$s3_staging
    }
    ...
    table <- paste0(
      quote_identifier(
        con,
        c(ll[["db.catalog"]], ll[["dbms.name"]], ll[["table"]])
      ),
      collapse = "."
    )
...
createFields <- function(con, fields, field.types) {
  ...
  if (!is.null(field.types)) {
    names(field.types) <- gsub(
      "\\.",
      "_",
      make.names(names(field.types), unique = TRUE)
    )
...
Compress <- function(file.type, compress) {
  if (compress) {
    switch(
      ...
      "json" = {
        info_msg("json format currently doesn't support compression")
        file.type
      }
    )
  } else {
...
repair_table <- function(
  ...
  if (is.null(partition)) {
    ...
    if (res@info[["Status"]] == "FAILED") {
      if (
        grepl(
          ".*glue.*BatchCreatePartition.*AccessDeniedException",
