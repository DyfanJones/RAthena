...
#' @keywords internal
#' @export
AthenaListObjects <- function(connection, ...) {
  UseMethod("AthenaListObjects")
}

#'@export
AthenaListObjects.AthenaConnection <- function(
  connection,
  catalog = NULL,
  schema = NULL,
  name = NULL,
...
computeHostName <- function(connection) {
  paste(
    ...
    c(
      ...
      "Athena",
      connection@ptr$region_name
    )
  )
}

computeDisplayName <- function(connection) {
  paste0("AWS Region: ", connection@ptr$region_name)
}
...
#' @export
AthenaListColumns.AthenaConnection <- function(
  connection,
  table = NULL,
  view = NULL,
  catalog = NULL,
...
#' @keywords internal
#' @export
AthenaPreviewObject <- function(connection, rowLimit, ...) {
  UseMethod("AthenaPreviewObject")
}

#' @export
AthenaPreviewObject.AthenaConnection <- function(
  connection,
  rowLimit,
  table = NULL,
  view = NULL,
  ...
  actions <- c(
    ...
    list(
      Help = list(
        ...
        callback = function() {
          utils::browseURL("https://dyfanjones.github.io/RAthena/")
        }
      )
    )
  )
...
on_connection_opened <- function(connection) {
  ...
  observer$connectionOpened(
    ...
    # connection code
    connectCode = paste(
      c("library(DBI)", "con <- dbConnect(RAthena::athena())"),
      collapse = "\n"
    ),
