...
#' \href{https://docs.aws.amazon.com/IAM/latest/UserGuide/reference-arns.html}{Amazon Resource Name roles} or environmental variables. Here is a list
#' of supported environment variables:
#'
#' - \strong{AWS_ACCESS_KEY_ID:} is equivalent to the \code{dbConnect} parameter - \code{aws_access_key_id}
#' - \strong{AWS_SECRET_ACCESS_KEY:} is equivalent to the \code{dbConnect} parameter - \code{aws_secret_access_key}
#' - \strong{AWS_SESSION_TOKEN:} is equivalent to the \code{dbConnect} parameter - \code{aws_session_token}
#' - \strong{AWS_EXPIRATION:} is equivalent to the \code{dbConnect} parameter - \code{duration_seconds}
#' - \strong{AWS_ATHENA_S3_STAGING_DIR:} is equivalent to the \code{dbConnect} parameter - \code{s3_staging_dir}
#' - \strong{AWS_ATHENA_WORK_GROUP:} is equivalent to \code{dbConnect} parameter - \code{work_group}
#' - \strong{AWS_REGION:} is equivalent to \code{dbConnect} parameter - \code{region_name}
#'
...
#'                          Supported Amazon S3 Encryption Options `c("NULL", "SSE_S3", "SSE_KMS", "CSE_KMS")`. Connection will default to NULL,
...
#'
#' - \strong{boto3.session.Session}
#'   - \strong{botocore_session} (botocore.session.Session): Use this Botocore session instead
#'             of creating a new default one.
#' - \strong{boto3.client}
#'   - \strong{config} (botocore.client.Config) -- Advanced client configuration options. If region_name
...
#'             for more details.
#'   - \strong{api_version} (string) -- The API version to use. By default, botocore will use the latest
...
#'   - \strong{use_ssl} (boolean) -- Whether or not to use SSL. By default, SSL is used. Note that
#'             not all services support non-ssl connections.
#'   - \strong{verify} (boolean/string) -- Whether or not to verify SSL certificates. By default
...
setMethod(
  ...
  function(
    ...
    if (!py_module_available("boto3")) {
      stop(
        "Boto3 is not detected please install boto3 using either: `pip install boto3 numpy` in terminal or `install_boto()`.",
