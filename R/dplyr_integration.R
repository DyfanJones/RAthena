...
#' @param name Table name, if left default RAthena will use the default from \code{dplyr}'s \code{compute} function.
...
#' @param file_type What file type to store data.frame on s3, RAthena currently supports \code{c("tsv", "csv", "parquet")}. Default delimited file type is "tsv", in previous versions
...
#' }
#' @export
db_compute.AthenaConnection <- function(
  con,
  table,
  sql,
  ...,
...
.sqlCreateTable <- function(
  ...
  if (inherits(name, "dbplyr_table_path")) {
    table_path_name <- pkg_method("table_path_name", "dbplyr")
    name <- table_path_name(name, con)
  } else {
...
athena_query_fields_ident <- function(con, sql) {
  if (str_count(sql, "\\.") < 2) {
    ...
    tryCatch(
      output <- py_to_r(con@ptr$Athena$get_table_metadata(
